...
def add_cumulative_estimators_pred(d):
    d["n"] = np.array(list(range(d.shape[0]))) + 1
    d["cum_obs_cnt"] = np.cumsum(d["obs"])
    d["cum_obs_val"] = np.cumsum(d["obs"] * d["y"].astype(float))
    d["obs_mean"] = d["cum_obs_val"] / d["cum_obs_cnt"]
    d["weight_pred"] = 1.0 / d["obs_prob_pred"]
    d["cum_obs_weight_pred"] = np.cumsum(d["obs"] * d["weight_pred"])
    d["cum_obs_weighted_y_pred"] = np.cumsum(d["obs"] * d["weight_pred"] * d["y"])
    d["ipw_est_pred"] = d["cum_obs_weighted_y_pred"] / d["n"]
    d["ipw_est_b_pred"] = d["cum_obs_weighted_y_pred"] / d["cum_obs_weight_pred"]
    d["ipw_est_b_b_pred"] = d["cum_obs_weighted_y_pred"] * d["n"] / d["cum_obs_weight_pred"] ** 2
    d["ipw_est_b3_pred"] = d["cum_obs_weighted_y_pred"] * d["n"] ** 2 / d["cum_obs_weight_pred"] ** 3
    return d
...
def add_mean_estimators_pred(d):
    ## Assumes columns created by genderate_data.get_missing_data_sample: "obs", "y", "x", "obs_prob"
    ## ONLY WORKS when x is a single column.  get_mean_estimators is more general...
    ...
    d["weight_pred"] = 1 / d["obs_prob_pred"]
    d["weight_obs_pred"] = d["obs"] * d["weight_pred"]
    d["weight_obs_val_pred"] = d["weight_obs_pred"] * d["y"]
    
    sum_obs_weight_pred = np.sum(d["weight_obs_pred"] )
    sum_obs_weighted_y_pred = np.sum(d["weight_obs_val_pred"])
    est["sum_obs_weight_pred"] = sum_obs_weight_pred
    est["ipw_mean_pred"] = sum_obs_weighted_y_pred / n
    est["ipw_b_mean_pred"] = sum_obs_weighted_y_pred / sum_obs_weight_pred
    est["ipw_b_b_mean_pred"] = sum_obs_weighted_y_pred * n / sum_obs_weight_pred ** 2
    ...
    missing_impute = full_impute.copy()
    missing_impute[d["obs"]] = d[d["obs"]]["y"]  # don't impute the observed values
    est["impute_missing_pred"] = np.mean(missing_impute)

    # Do IPW regression imputation
    reg = LinearRegression(fit_intercept=True, normalize=False).fit(
        X=d["x"], y=d["y"], sample_weight=d["weight_obs_pred"]
    )
    full_impute_w = reg.predict(d["x"])
    missing_impute_w = full_impute_w.copy()
    missing_impute_w[d["obs"]] = d[d["obs"]]["y"]  # don't impute the observed values
    est["impute_missing_w_pred"] = np.mean(missing_impute_w)

    # Do doubly robust estimate
    beta = 1
    d["dr_vec_pred"] = d["weight_obs_val_pred"] - beta * (d["weight_obs_pred"] - 1) * full_impute
    d["dr_vec_w_pred"] = d["weight_obs_val_pred"] - beta *(d["weight_obs_pred"] - 1) * full_impute_w
    est["ipw_dr_1_pred"] = np.sum(d["dr_vec_pred"])/n
    est["ipw_dr_w_1_pred"] = np.sum(d["dr_vec_w_pred"])/n

    f_centered = d["weight_obs_val_pred"] - np.mean(d["weight_obs_val_pred"])
    h_centered  = d["weight_obs_pred"] * full_impute - np.mean(d["weight_obs_pred"] * full_impute)
    ...
    d["dr_vec_opt_pred"] = d["weight_obs_val_pred"] - beta * (d["weight_obs_pred"] - 1) * full_impute
    d["dr_vec_w_opt_pred"] = d["weight_obs_val_pred"] - beta *(d["weight_obs_pred"] - 1) * full_impute_w
    est["ipw_dr_opt_pred"] = np.sum(d["dr_vec_opt_pred"])/n
    est["ipw_dr_w_opt_pred"] = np.sum(d["dr_vec_w_opt_pred"])/n

    est["ipw_dr_b_pred"] = np.sum(dr_vec) * est["weight_pred"] / n**2
    est["ipw_dr_w_b_pred"] = np.sum(dr_vec_w) * est["weight_pred"] / n**2
...
def get_mean_estimators_pred(vals, covar, propensities, obs):
    est = {}
    n = len(vals)
    ## All the estimators below assume we know the propensities (i.e. "missing by design")
    est["obs_mean_pred"] = np.mean(vals[obs])
    est["ipw_mean_pred"] = np.sum(vals[obs] / propensities[obs]) / n
    est["ipw_b_mean_pred"] = np.average(vals[obs], weights=1 / propensities[obs])
    est["weight_pred"] = np.sum(1 / propensities[obs])
    est["ipw_b_b_mean_pred"] = (
        np.average(vals[obs], weights=1 / propensities[obs]) * n / est["weight_pred"]
    )
    est["ipw_b3_mean_pred"] = (
        np.average(vals[obs], weights=1 / propensities[obs])
        * n ** 2
        / est["weight_pred"] ** 2
    )
    est["num_obs_pred"] = np.sum(obs)
    ...
    missing_impute = full_impute_w.copy()
    missing_impute[obs] = vals[obs]  # don't impute the observed values
    est["impute_missing_w_pred"] = np.mean(missing_impute)
    ...
    dr_vec = WRY - beta * (weights * obs - 1) * full_impute
    dr_vec_w = WRY - beta *(weights * obs - 1) * full_impute_w
    est["ipw_dr_1_pred"] = np.sum(dr_vec)/n
    est["ipw_dr_w_1_pred"] = np.sum(dr_vec_w)/n

    beta = 1
    dr_vec = WRY - beta * (weights * obs - 1) * full_impute
    dr_vec_w = WRY - beta *(weights * obs - 1) * full_impute_w

    est["ipw_dr_opt_pred"] = np.sum(dr_vec)/n
    est["ipw_dr_w_opt_pred"] = np.sum(dr_vec_w)/n
